...
public class Logger {
  ...
  public void log(String msg, Level level) {
    this.stream.format("[%s][%s][%s] %s\n", new Date(), this.className.substring(this.className.lastIndexOf('.') + 1), level.getName(), msg);
  }
  ...
  private static class OutStream extends PrintStream {
    ...
    private void addCallerName() {
      ...
      super.append(String.format("[%s/%s(%s):%s] ", className, e.getMethodName(), line >= 0 ? "" + line : "?", this.out == OUT ? "OUT" : "ERR"));
    }
  }
}
